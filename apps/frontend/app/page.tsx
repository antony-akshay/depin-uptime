...
function App() {
  ...
  // Process raw website data into the format expected by components
  const websites: ProcessedWebsite[] = rawWebsites.map(website => ({
    id: website.id,
    name: new URL(website.url).hostname, // Extract domain name as website name
    url: website.url,
  ...
  const handleCreateWebsite = (data: { name: string; url: string }) => {
    // TODO: Implement API call to create new website
    console.log('Creating website:', data);
    // For now, just close the modal - the useWebsites hook will refresh automatically
  };
  ...
  const overallStats = {
    ...
    avgUptime: websites.length > 0 
      ? Math.round(websites.reduce((acc, w) => acc + w.uptimePercentage, 0) / websites.length * 10) / 10 
      : 0
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      <Header 
        ...
        onCreateWebsite={() => setIsModalOpen(true)}
      />
